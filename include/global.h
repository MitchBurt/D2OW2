...
//Configuration
#define CURRENT_GAME_VERSION    0
#define CARD_CURRENT_VERSION 	CARD_GNORB

#define FLAGS_TO_CARRY_COUNT    SIDE_QUEST_COUNT
...
struct SaveBlock2
{
	...
	/*0x????*/ struct PokemonFollower pokemonFollower;
	/*0x????*/ u8 unlockedQuests[SIDE_QUEST_FLAGS_COUNT];
	/*0x????*/ u8 completedQuests[SIDE_QUEST_FLAGS_COUNT];
	/*0x????*/ u8 titleScreen:4;
	/*0x????*/ u8 optionsDamageDoneMessageEnabled:1;
	/*0x????*/ u8 optionsGenericCry:1;
	/*0x????*/ u8 optionsBikeType:1;
	/*0x????*/ u8 optionsEnableEmuCheck:1;
	/*0x????*/ u8 optionsMgbaMode:1;
	/*0x????*/ bool8 roamerFlag[ROAMER_COUNT];
}; // sizeof=0x???
 ...
 struct WonderCard
{
	...
	u16 flagId;  														//u16 unk_00;
    u16 iconSpecies; 													//u16 unk_02;
    u32 idNumber; 														//u32 unk_04;
    u8 type:2; 	// CARD_TYPE_* 											//u8 unk_08_0:2;
    u8 bgType:4; 														//u8 unk_08_2:4;
    u8 sendType:2; // SEND_TYPE_* 										//u8 unk_08_6:2;
    u8 maxStamps; 														//u8 unk_09;
    u8 titleText[WONDER_CARD_TEXT_LENGTH]; 								//u8 unk_0A[40];
    u8 subtitleText[WONDER_CARD_TEXT_LENGTH]; 							//u8 unk_32[40];
    u8 bodyText[WONDER_CARD_BODY_TEXT_LINES][WONDER_CARD_TEXT_LENGTH];  //u8 unk_5A[4][40];
    u8 footerLine1Text[WONDER_CARD_TEXT_LENGTH]; 						//u8 unk_FA[40];
    u8 footerLine2Text[WONDER_CARD_TEXT_LENGTH]; 						//u8 unk_122[40];
};
...
//#define MAX_ACTIVE_CHARMS 10
//extern u16 gActiveCharms[MAX_ACTIVE_CHARMS]; // Declare the array globally

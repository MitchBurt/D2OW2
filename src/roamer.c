...
void CreateOnlineRoamer(u16 species, u8 level)
{
    ClearRoamerData();
    ClearRoamerLocationData();
    (&gSaveBlock1Ptr->roamer)->species = species;

    CreateMon(&gEnemyParty[0], (&gSaveBlock1Ptr->roamer)->species, level, 0x20, 0, 0, OT_ID_PLAYER_ID, 0, 0);
    (&gSaveBlock1Ptr->roamer)->level       = level;

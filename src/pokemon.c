...
const u16 gSpeciesToNationalPokedexNum[NUM_SPECIES] = // Assigns all species to the National Dex Index (Summary No. for National Dex)
{
    ...
    SPECIES_TO_NATIONAL(WORMADAM-T),
    SPECIES_TO_NATIONAL(ROCCO),
    SPECIES_TO_NATIONAL(ABOMINATION),
    ...
    SPECIES_TO_NATIONAL(OUROBOUTLET),
    SPECIES_TO_NATIONAL(PICHU),
    SPECIES_TO_NATIONAL(POOPRAT),
    SPECIES_TO_NATIONAL(VCR-BRAIN),
    ...
    SPECIES_TO_NATIONAL(AURICLIARY2),
    ...
    SPECIES_TO_NATIONAL(AURICLIARY_Z),
    ...
    SPECIES_TO_NATIONAL(PYUKURIMESLANCHU),
    ...
    SPECIES_TO_NATIONAL(ETERPUCKIDS),
    ...
    [SPECIES_NONONMON_ALOLAN - 1] = NATIONAL_DEX_NONONMON,
    ...
    [SPECIES_RIMESLANCH_ALOLAN - 1] = NATIONAL_DEX_RIMESLANCH,
    ...
    // Eterpuckids
    [SPECIES_ETERPUCKIDS_ETERNAMAX - 1] = NATIONAL_DEX_ETERPUCKIDS,
    // Urshifu
...
const s8 gPuckidreStatTable[NUM_PUCKIDRES][NUM_PUCKIDRE_STATS] =
{
...
static const u8 sMonFrontAnimIdsTable[NUM_SPECIES - 1] =
{
    ...
    [SPECIES_PYUKURIMESLANCHU - 1] = 0x00,
...
static const u8 sMonAnimationDelayTable[NUM_SPECIES - 1] =
{
	...
	[SPECIES_QWILFISH_HUSIAN - 1] = 0x27,
...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId, u8 formId)
{
	...
	value = HIDDEN_PUCKIDRE_NONE;
    SetBoxMonData(boxMon, MON_DATA_HIDDEN_PUCKIDRE, &value);
...
void CreateMonWithPuckidre(struct Pokemon *mon, u16 species, u8 level, u8 fixedIV, u8 puckidre, u8 formId)
{
    u32 personality;
    ...
    while (puckidre != GetPuckidreFromPersonality(personality));

    CreateMon(mon, species, level, fixedIV, 1, personality, OT_ID_PLAYER_ID, 0, formId);
}

void CreateMonWithGenderPuckidreLetter(struct Pokemon *mon, u16 species, u8 level, u8 fixedIV, u8 gender, u8 puckidre, u8 unownLetter, u8 formId)
{
    u32 personality;
    u16 formSpeciesId = GetFormSpeciesId(species, formId);
...
void CreateMonWithEVSpreadPuckidreOTID(struct Pokemon *mon, u16 species, u8 level, u8 puckidre, u8 fixedIV, u8 evSpread, u32 otId, u8 formId)
{
    s32 i;
    s32 statCount = 0;
    u8 evsBits;
    ...
    {
        i = Random32();
    } while (puckidre != GetPuckidreFromPersonality(i));
...
#define SPECIAL_FARIGIRAF           SPECIES_GIRAFARIG
#define SPECIAL_DUNDUNSPARCE        SPECIES_DUNSPARCE
...
{                                                               \
    ...
    u8 puckidre = GetPuckidre(mon, TRUE);                                 \
    n = ModifyStatByPuckidre(puckidre, n, statIndex);               \
...
void CalculateMonStats(struct Pokemon *mon)
{
	...
	if(exioliteStatus == 1 && !FlagGet(FLAG_NO_EVOLUTION_MODE)){
		...
		newAtk = ModifyStatByPuckidre(puckidre, n, STAT_ATK);
		...
		newDef = ModifyStatByPuckidre(puckidre, n, STAT_DEF);
		...
		newSpA = ModifyStatByPuckidre(puckidre, n, STAT_SPATK);
		...
		newSpD = ModifyStatByPuckidre(puckidre, n, STAT_SPDEF);
		...
		newSpe = ModifyStatByPuckidre(puckidre, n, STAT_SPEED);
	}
...
u32 GetBoxMonData(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    case MON_DATA_RIBBONS:
        ...
        break;
	case MON_DATA_HIDDEN_PUCKIDRE:
        retVal = substruct0->hidden_puckidre;
        break;
...
void SetBoxMonData(struct BoxPokemon *boxMon, s32 field, const void *dataArg)
{
	...
	case MON_DATA_HIDDEN_PUCKIDRE:
        SET8(substruct0->hidden_puckidre);
        break;
	case MON_DATA_EXIOLITE_ENABLED:
...
u8 GetPuckidre(struct Pokemon *mon, bool32 checkHidden)
{
	if (!checkHidden || GetMonData(mon, MON_DATA_HIDDEN_PUCKIDRE, 0) == HIDDEN_PUCKIDRE_NONE || GetMonData(mon, MON_DATA_HIDDEN_PUCKIDRE, 0) == PUCKIDRE_HARDY)
        return GetPuckidreFromPersonality(GetMonData(mon, MON_DATA_PERSONALITY, 0));
    else
        return GetMonData(mon, MON_DATA_HIDDEN_PUCKIDRE, 0);
}

u8 GetPuckidreFromPersonality(u32 personality)
{
    return personality % NUM_PUCKIDRES;
}
				...
				if (gEvolutionTable[species][i].param <= level)
                {
					...
					if(gEvolutionTable[species][i].targetSpecies == SPECIES_TOXTRICITY){
						switch (puckidre)
						{
							case PUCKIDRE_HARDY:
							case PUCKIDRE_BRAVE:
							case PUCKIDRE_ADAMANT:
							case PUCKIDRE_NAUGHTY:
							case PUCKIDRE_DOCILE:
							case PUCKIDRE_IMPISH:
							case PUCKIDRE_LAX:
							case PUCKIDRE_HASTY:
							case PUCKIDRE_JOLLY:
							case PUCKIDRE_NAIVE:
							case PUCKIDRE_RASH:
							case PUCKIDRE_SASSY:
							case PUCKIDRE_QUIRKY:
								targetSpecies = SPECIES_TOXTRICITY;
								break;
							case PUCKIDRE_LONELY:
							case PUCKIDRE_BOLD:
							case PUCKIDRE_RELAXED:
							case PUCKIDRE_TIMID:
							case PUCKIDRE_SERIOUS:
							case PUCKIDRE_MODEST:
							case PUCKIDRE_MILD:
							case PUCKIDRE_QUIET:
							case PUCKIDRE_BASHFUL:
							case PUCKIDRE_CALM:
							case PUCKIDRE_GENTLE:
							case PUCKIDRE_CAREFUL:
...
u16 ModifyStatByPuckidre(u8 puckidre, u16 n, u8 statIndex)
{
    u32 retVal;
    // Don't modify HP, Accuracy, or Evasion by puckidre
    if (statIndex <= STAT_HP || statIndex > NUM_PUCKIDRE_STATS)
    {
        return n;
    }

    switch (gPuckidreStatTable[puckidre][statIndex - 1])
    {
    case 1:
        retVal = n * 110;
        retVal /= 100;
...
{
    u8 puckidre = GetPuckidre(mon, FALSE);
    return gPokeblockFlavorCompatibilityTable[puckidre * FLAVOR_COUNT + flavor];
}

s8 GetFlavorRelationByPersonality(u32 personality, u8 flavor)
{
    u8 puckidre = GetPuckidreFromPersonality(personality);
    return gPokeblockFlavorCompatibilityTable[puckidre * FLAVOR_COUNT + flavor];
}
...
void CreateShinyMonWithPuckidre(struct Pokemon *mon, u16 species, u8 level, u8 puckidre)
{
    u32 personality;
...
u16 GetRandomFirstStage(u16 basespecies){
    ...
    if(basespecies == SPECIES_AUDINO     || basespecies == SPECIES_RELICANTH || 
       ...
       basespecies == SPECIES_SUNKERN    || basespecies == SPECIES_SUNFLORA  || 

...
static const u16 sHallOfFame_Pal[] = INCBIN_U16("graphics/ui_menu/hall_of_fame/hall_of_fame.gbapal");

static const u32 sHallOfFame_Gfx[] = INCBIN_U32("graphics/ui_menu/hall_of_fame/hall_of_fame.4bpp.lz");
...
static void Task_Hof_DisplayMon(u8 taskId)
{
    ...
    spriteId = CreatePicSprite2(currMon->species, currMon->tid, currMon->personality, 1, startX, startY, currMonId, 0xFFFF, formId); // handle forms, Hall of Fame struct needs to be modified
...
static void Task_Hof_TryDisplayAnotherMon(u8 taskId)
{
    ...
    else
    {
        ...
        if (gTasks[taskId].tDisplayedMonId < PARTY_SIZE - 1 && currMon[1].species != SPECIES_NONE) // there is another pokemon to display
        {
            ...
            BeginNormalPaletteFade(sHofFadePalettes, 0, 12, 12, RGB_BLACK);
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId)
{
    ...
    BlendPalettes(0xFFFF0000, 0xC, RGB_BLACK);
...
static void Task_HofPC_PrintMonInfo(u8 taskId)
{
    ...
    BlendPalettesUnfaded(sHofFadePalettes, 0xC, RGB_BLACK);
...
static void HallOfFame_PrintWelcomeText(u8 unusedPossiblyWindowId, u8 unused2)
{
	...
	static const u8 InversemodeText[] 			= _("Inverse Mode ");
	static const u8 RandomizedmodeText[] 		= _("Randomized Mode");
	static const u8 RandomPartymodeText[] 		= _("Random Party Mode");
	static const u8 LosesText[] 				= _("Number of Losses: ");
	static const u8 EmptyText[] 				= _("");
	...
	if (FlagGet(FLAG_INVERSE_MODE))
		StringCopy(gStringVar2, InversemodeText);
	else
		StringCopy(gStringVar2, EmptyText);
	
	//Modes 2
	if (FlagGet(FLAG_RANDOMIZED_MODE))
		StringCopy(gStringVar3, RandomizedmodeText);
	else if(FlagGet(FLAG_FULL_RANDOMIZED_MODE))
		StringCopy(gStringVar3, RandomPartymodeText);
	else
		StringCopy(gStringVar2, EmptyText);
	
	//If no Mode is enabled show the Number of whiteouts
	if(!FlagGet(FLAG_RANDOMIZED_MODE) && !FlagGet(FLAG_INVERSE_MODE) && !FlagGet(FLAG_FULL_RANDOMIZED_MODE)){
		StringCopy(gStringVar2, LosesText);
	...
	FillWindowPixelBuffer(0, PIXEL_FILL(0));
    PutWindowTilemap(0);
    AddTextPrinterParameterized4(0, 2, GetStringCenterAlignXOffset(1, gStringVar4, 0xD0), 1, 0, 0, sMonInfoTextColors, 0, gStringVar4);
	CopyWindowToVram(0, 3);
}

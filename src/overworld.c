...
static void Overworld_ResetStateAfterWhiteOut(void)
{
    ...
    FlagClear(FLAG_SYS_USE_STRENGTH);
    FlagClear(FLAG_SYS_USE_FLASH);
    // If you were defeated by Krakrum/Noirnet and the step counter has
...
#define RUSTBORO_GYM   0
#define DEWFORD_GYM    1
#define MAUVILLE_GYM   2
#define LAVARIDGE_GYM  3
#define PETALBURG_GYM  4
#define FORTREE_GYM    5
#define MOSSDEEP_GYM   6
#define SOOTOPOLIS_GYM 7
#define NUM_GYMS       8

const struct WarpData gGymWarpData[] = 
{
	[RUSTBORO_GYM] =
    {
...
const struct WarpData gGymExitWarpData[] = 
{
	[RUSTBORO_GYM] =
    {
...
void ReShuffleGyms(void){
	u8 i,j;
	u8 gymList[] = {NUM_GYMS, NUM_GYMS, NUM_GYMS, NUM_GYMS, NUM_GYMS, NUM_GYMS, NUM_GYMS, NUM_GYMS};
	bool8 hasPassedCheck;
	
	u16 sGymVars[] = {	//Eight Badges
		RUSTBORO_GYM_SHUFFLE_NUM,
		DEWFORD_GYM_SHUFFLE_NUM,
		MAUVILLE_GYM_SHUFFLE_NUM,
		LAVARIDGE_GYM_SHUFFLE_NUM,
		PETALBURG_GYM_SHUFFLE_NUM,
		FORTREE_GYM_SHUFFLE_NUM,
		MOSSDEEP_GYM_SHUFFLE_NUM,
		SOOTOPOLIS_GYM_SHUFFLE_NUM,
	};
	
	for (i = 0; i < NUM_GYMS; i++)
    {
		switch(i){
			case 0:
				gymList[i] = (RandRange(0, NUM_GYMS)) % NUM_GYMS;
			break;
			case 1:
				do
				{
					gymList[i] = (RandRange(0, NUM_GYMS)) % NUM_GYMS;
				}while(gymList[i] == gymList[0]);
			break;
			case 2:
				do
				{
					gymList[i] = (RandRange(0, NUM_GYMS)) % NUM_GYMS;
				}while(gymList[i] == gymList[0] || gymList[i] == gymList[1]);
			break;
			case 3:
				do
				{
	...
	for (i = 0; i < NUM_GYMS; i++)
    {
		VarSet(sGymVars[i], gymList[i]);
	}
	
	FlagSet(FLAG_GYM_SHUFFLED);
}
...
void SetWarpDestinationToMapWarp(s8 mapGroup, s8 mapNum, s8 warpId)
{
	if(FlagGet(FLAG_GYM_SHUFFLE_MODE)){
		...
		if(!FlagGet(FLAG_GYM_SHUFFLED))
			ReShuffleGyms();
		
		//Gym Entrances ------------------------------------------------------------------------------------------------------------
		for(i = 0; i < NUM_GYMS; i++){
			if(mapGroup == gGymWarpData[i].mapGroup && 
			   mapNum   == gGymWarpData[i].mapNum   && 
			   warpId   == gGymWarpData[i].warpId   &&
			   gymFound == FALSE)
			{
				gymNum = VarGet(sGymVars[i]);
				VarSet(LAST_GYM_NUM, i);
				
				mapGroup = gGymWarpData[gymNum].mapGroup;
				mapNum   = gGymWarpData[gymNum].mapNum;
				gymFound = TRUE;
			}
		}
		...
		//Gym Exits
		for(i = 0; i < NUM_GYMS; i++){
			if(mapGroup == gGymExitWarpData[i].mapGroup && 
			   mapNum   == gGymExitWarpData[i].mapNum   && 
			   warpId   == gGymExitWarpData[i].warpId   &&
			   gymFound == FALSE)
			{
				gymNum = VarGet(LAST_GYM_NUM);
					
				mapGroup = gGymExitWarpData[gymNum].mapGroup;
				mapNum   = gGymExitWarpData[gymNum].mapNum;
				warpId   = gGymExitWarpData[gymNum].warpId;
				gymFound = TRUE;
			}
		}
	}
...
static void ResumeMap(bool32 a1)
{
    ...
    FreeAllSpritePalettes();//tx_pokemon_follower
...
static void sub_8086A68(void)//InitObjectEventsReturnToField
{
    sub_808E16C(0, 0);

...
static void DrawDexNavSearchMonIcon(u16 species, u8 *dst, bool8 owned)
{
    ...
    spriteId = CreateMonIcon(species, SpriteCB_MonIcon, SPECIES_ICON_X - 6, GetSearchWindowY() + 8, 0, 0, formId);
...
static void AddSearchWindowText(u16 species, u8 proximity, u8 searchLevel, bool8 hidden)
{
    ...
    ConvertIntToDecimalStringN(gStringVar1, VarGet(VAR_DEXNAV_CHAIN), STR_CONV_MODE_LEFT_ALIGN, 3);
    if (VarGet(VAR_DEXNAV_CHAIN) > 99)
...
static void Task_SetUpDexNavSearch(u8 taskId)
{
    ...
    u8 searchLevel = VarGet(VAR_DEXNAV_CHAIN);
...
static void DexNavDrawIcons(void)
{
	...
	if(FlagGet(FLAG_RANDOMIZED_MODE) && !FlagGet(FLAG_FULL_RANDOMIZED_MODE) && IsPokemonValid(GetRandomFirstStage(species)))
			species = GetRandomFirstStage(species);
...
{
    if (VarGet(VAR_DEXNAV_CHAIN) < 20)
        return 2;
    else if (VarGet(VAR_DEXNAV_CHAIN) < 50)
        return 3;
    else
        return 4;
}
...
static void DexNavUpdateSearchWindow(u8 proximity, u8 searchLevel)
{
    ...
    if (sDexNavSearchDataPtr->hiddenSearch && !GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_SEEN))
...
static u8 DexNavTryGenerateMonLevel(u16 species, u8 environment)
{
    ...
    u8 levelBonus = VarGet(VAR_DEXNAV_CHAIN) / 5;
	u8 level = getMinWildPokemonLevel();
	u8 dexnavMaxLevel = getGymLeaderMinLevel();
    ...
    if (level + levelBonus > dexnavMaxLevel)
        return dexnavMaxLevel;
    else
...
static void DexNavGenerateMoveset(u16 species, u8 searchLevel, u8 encounterLevel, u16* moveDst)
{
	...
	//if(FlagGet(FLAG_RANDOMIZED_MODE) && species != SPECIES_NONE && IsPokemonValid(GetRandomFirstStage(species)))
	//	species = GetRandomFirstStage(species);
...
static void DrawSpeciesIcons(void)
{
    ...
    for (i = 0; i < HIDDEN_WILD_COUNT; i++)
    {
        ...
        if (FlagGet(FLAG_SYS_DEXNAV_GET))
            ...
            CreateMonIcon(SPECIES_NONE, SpriteCB_MonIcon, x, y, 0, 0, formId); //question mark if detector mode inactive
    }
}
...
static u16 DexNavGetSpecies(void)
{
    ...
    if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_SEEN) && !FlagGet(FLAG_RANDOMIZED_MODE))
        return SPECIES_NONE;
	else if(!GetSetPokedexFlag(SpeciesToNationalPokedexNum(GetRandomFirstStage(species)), FLAG_GET_SEEN) && FlagGet(FLAG_RANDOMIZED_MODE))
		return SPECIES_NONE;
...
static void PrintCurrentSpeciesInfo(void)
{
    ...
    {
        ConvertIntToDecimalStringN(gStringVar4, VarGet(VAR_DEXNAV_CHAIN), 0, 4);
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, SEARCH_LEVEL_Y, sFontColor_Black, 0, gStringVar4);
    }
    
    // search bonus
    if ((VarGet(VAR_DEXNAV_CHAIN) >> 2) > 20)
        searchLevelBonus = 20;
    else
        searchLevelBonus = (VarGet(VAR_DEXNAV_CHAIN) >> 2);
...
void Task_OpenDexNavFromStartMenu(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        DexNavGuiInit(CB2_ReturnToUIMenu); //CB2_ReturnToFieldWithOpenMenu
        DestroyTask(taskId);
    }
}
...
static void Task_DexNavMain(u8 taskId)
{
    ...
    else if (JOY_NEW(R_BUTTON))
    {
        ...
        {            
			...
			if(FlagGet(FLAG_RANDOMIZED_MODE) && species != SPECIES_NONE && IsPokemonValid(GetRandomFirstStage(species)))
				PlayCry5(GetRandomFirstStage(species), 0);
			else
				PlayCry5(species, 0);
...
static void DrawHiddenSearchWindow(u8 width)
{
    ...
    ConvertIntToDecimalStringN(gStringVar1, VarGet(VAR_DEXNAV_CHAIN), STR_CONV_MODE_LEFT_ALIGN, 2);
...
bool8 DexNavTryMakeShinyMon(void)
{
    ...
    u32 shinyRate = 0;
    u32 charmBonus = 0;
    u8 chain = VarGet(VAR_DEXNAV_CHAIN);
    
    charmBonus = (CheckBagHasItem(ITEM_SHINY_CHARM, 1) > 0) ? 1 : 0;
	chainBonus = (chain % 10 == 0 || chain > 250) ? 1 : 0;
...
{
    u16 newChain = VarGet(VAR_DEXNAV_CHAIN) + 1;
    if (VarGet(VAR_DEXNAV_CHAIN) < DEXNAV_CHAIN_MAX)
        VarSet(VAR_DEXNAV_CHAIN, newChain);
}

void DecreaseDexNavChain(void)
{
    u16 newChain = VarGet(VAR_DEXNAV_CHAIN) - 1;
    if (VarGet(VAR_DEXNAV_CHAIN) != 0)
        VarSet(VAR_DEXNAV_CHAIN, newChain);
}
...
bool8 DaycareTryMakeShinyMon(void);
...
static void AlterEggSpeciesWithIncenseItem(u16 *species, struct DayCare *daycare)
{
    ...
    #endif
    #ifdef SPECIES_LUIWEE
        else if (*species == SPECIES_LUIWEE && motherItem != ITEM_PURE_INCENSE && fatherItem != ITEM_PURE_INCENSE)
...
static u8 GetMonFormId(u16 *species, struct DayCare *daycare){
	...
	switch(*species){
		...
		case SPECIES_MEOWRRIOR_GALARIAN:
			if(motherSpecies == SPECIES_OBSTAGOON || (fatherSpecies == SPECIES_OBSTAGOON && motherSpecies == SPECIES_DITTO))
				return GetFormIdFromFormSpeciesId(SPECIES_MEOWRRIOR_GALARIAN);
		break;
		case SPECIES_YAMASK:
		case SPECIES_YAMASK_GALARIAN:
...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots)
{
    ...
    if (eggSpecies == SPECIES_NAUSEON && daycare->offspringPersonality & EGG_GENDER_MALE)
    {
	...
	if (eggSpecies == SPECIES_INDEEDEE_FEMALE && (Random() % 3) >= 1)
    {
...
static void SetInitialEggData(struct Pokemon *mon, u16 species, struct DayCare *daycare)
{
	...
	u8 wantedNature = GetNatureFromPersonality(daycare->offspringPersonality);
	bool8 isShiny = FALSE;
	u8 parent = GetParentToInheritNature(daycare);
	u16 ball;
	
	//Gets the parent ball
	if(GetBoxMonData(&daycare->mons[0].mon, MON_DATA_POKEBALL, NULL) != ITEM_POKE_BALL && 
	   GetBoxMonData(&daycare->mons[0].mon, MON_DATA_POKEBALL, NULL) != ITEM_CHERISH_BALL)
		ball = GetBoxMonData(&daycare->mons[0].mon, MON_DATA_POKEBALL, NULL);
	else if(GetBoxMonData(&daycare->mons[1].mon, MON_DATA_POKEBALL, NULL) != ITEM_POKE_BALL && 
	        GetBoxMonData(&daycare->mons[1].mon, MON_DATA_POKEBALL, NULL) != ITEM_CHERISH_BALL)
		ball = GetBoxMonData(&daycare->mons[1].mon, MON_DATA_POKEBALL, NULL);
	else
		ball = ITEM_POKE_BALL;
			  
	personality = daycare->offspringPersonality;
	
	isShiny = DaycareTryMakeShinyMon();
	...
	if(isShiny && parent < 2 && GetNatureFromPersonality(personality) != wantedNature)
		SetMonData(mon, MON_DATA_HIDDEN_NATURE, &wantedNature);
    //SetMonData(mon, MON_DATA_LANGUAGE, &language);
...
static u8 GetDaycareCompatibilityScore(struct DayCare *daycare)
{
    ...
    u16 species[DAYCARE_MON_COUNT];
    u16 speciesFormId[DAYCARE_MON_COUNT];
    u32 trainerIds[DAYCARE_MON_COUNT];
    u32 genders[DAYCARE_MON_COUNT];
    u8  formID[DAYCARE_MON_COUNT];
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++)
    {
        ...
        species[i] = GetBoxMonData(&daycare->mons[i].mon, MON_DATA_SPECIES);
        formID[i] = GetBoxMonData(&daycare->mons[i].mon, MON_DATA_FORM_ID);
        ...
        speciesFormId[i] = GetFormSpeciesId(species[i], formID[i]);
        eggGroups[i][0] = gBaseStats[speciesFormId[i]].eggGroup1;
        eggGroups[i][1] = gBaseStats[speciesFormId[i]].eggGroup2;
    }

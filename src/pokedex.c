...
extern const u16 gBaseFormID[NUM_SPECIES];
...
EWRAM_DATA static u8  dexFormId = 0;
...
static u32 CreateDarkenedPokedexMonSprite(u16 num, s16 x, s16 y);
...
const u8 gText_EVO_LEVEL_NIGHT_ALOLA[]              = _("Lvl up to {STR_VAR_2} at night\nwhile in Sevii Islands");
const u8 gText_EVO_ITEM_ALOLA[]                     = _("{STR_VAR_2} is used on Pokémon\nwhile in Sevii Islands");
const u8 gText_EVO_MELMETAL[]                       = _("Lvl up to {STR_VAR_2} while having a party\nfull of Meltans");
...
void ResetPokedex(void)
{
    ...
    gSaveBlock2Ptr->pokedex.yeltzerPersonality = 0;
...
static void ResetPokedexView(struct PokedexView *pokedexView)
{
	...
	pokedexView->pokedexList[NATIONAL_DEX_COUNT].formId = 0;
...
static void Task_HandlePokedexInput(u8 taskId)
{
    ...
    {
        if ((JOY_NEW(A_BUTTON)))// && sPokedexView->pokedexList[sPokedexView->selectedPokemon].seen ASDF
        {
...
static void CreateMonSpritesAtPos(u16 selectedMon, u16 ignored)
{
	...
	if(dexNum == 0){
		dexNum =  sPokedexView->pokedexList[selectedMon - 1].dexNum;
		isNotSeen = TRUE;
	}
	
    if (dexNum != 0xFFFF && !isNotSeen)
    {
	...
	if(dexNum == 0){
		dexNum =  sPokedexView->pokedexList[selectedMon].dexNum;
		isNotSeen = TRUE;
	}
...
static u32 CreateDarkenedPokedexMonSprite(u16 num, s16 x, s16 y)
{
    ...
    for (i = 0; i < MAX_MONS_ON_SCREEN; i++)
    {
        if (sPokedexView->monSpriteIds[i] == 0xFFFF)
        {
            ...
            gSprites[spriteId].oam.affineMode = ST_OAM_AFFINE_NORMAL;
            gSprites[spriteId].oam.priority = 3;
			//gSprites[spriteId].oam.paletteNum = 255;
...
{
	u16 formSpeciesId = GetFormSpeciesId(sPokedexListItem->dexNum, dexFormId);
	u32 seen = sPokedexListItem->seen;
    switch (gMain.state)
    {
    case 0:
    default:
    ...
    case 5:
        if (gTasks[taskId].data[1] == 0)
        {
            gTasks[taskId].tMonSpriteId = (u16)CreateMonSpriteFromNationalDexNumber(formSpeciesId, 48, 56, 0);
            gSprites[gTasks[taskId].tMonSpriteId].oam.priority = 0;
			
			if(!seen)
				LoadPalette(sSizeScreenSilhouette_Pal, (gSprites[gTasks[taskId].tMonSpriteId].oam.paletteNum + 16) * 16, 0x20);
        }
		gMain.state++;
        break;
    case 6:
...
{
	u16 formSpeciesId = GetFormSpeciesId(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), dexFormId);
    switch (gMain.state)
    {
    case 0:
    default:
    ...
    case 1:
        ...
        break;
    case 2:
        ShowPokedexAreaScreen(formSpeciesId, &sPokedexView->screenSwitchState);
        SetVBlankCallback(gPokedexVBlankCB);
        sPokedexView->screenSwitchState = 0;
        gMain.state = 0;
...
static void Task_LoadCryScreen(u8 taskId)
{
    ...
    case 4:
        ...
        break;
    case 5:
        gTasks[taskId].tMonSpriteId = CreateMonSpriteFromNationalDexNumber(formSpeciesId, 48, 56, 0);
        gSprites[gTasks[taskId].tMonSpriteId].oam.priority = 0;
...
static void PrintCurrentSpeciesTypeInfo(void)
{
	...
	u16 formSpeciesId = GetFormSpeciesId(species, dexFormId);
    u32 i;
    ...
    type1 = gBaseStats[formSpeciesId].type1;
    type2 = gBaseStats[formSpeciesId].type2;
	}else{
	type1 = gVanillaBaseStats[formSpeciesId].type1;
    type2 = gVanillaBaseStats[formSpeciesId].type2;	
	}
		
    if (formSpeciesId == SPECIES_NONE)
...
bool16 HasAllHoennMons(void)
{
    ...
    // -2 excludes Thiscraker and Squilithid
...
// Unown and Yeltzer use the personality of the first seen individual of that species
// All others use personality 0
static u32 GetPokedexMonPersonality(u16 species)
{
    if (species == SPECIES_UNOWN || species == SPECIES_YELTZER)
    {
        if (species == SPECIES_UNOWN)
            return gSaveBlock2Ptr->pokedex.unownPersonality;
        else
            return gSaveBlock2Ptr->pokedex.yeltzerPersonality;
    }
    else
    {
        return 0;
...
{
	//u16 formId = GetFormIdFromFormSpeciesId(nationalNum);
	...
	//return CreatePicSprite2(species, otId, personality, flags, x, y, paletteSlot, paletteTag, formId);
}
            ...
            for (i = 0, resultsCount = 0; i < sPokedexView->pokemonListCount; i++)
            {
                if (sPokedexView->pokedexList[i].owned)
                {
                    ...
                    if(!FlagGet(FLAG_VANILLA_MODE)){
						...
						types[0] = gVanillaBaseStats[species].type1;
						types[1] = gVanillaBaseStats[species].type2;
					}
...
static void Task_LoadStatsScreen(u8 taskId)
{
    ...
    case 5:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            LoadMonIconPalette(GetFormSpeciesId(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), dexFormId)); //Loads pallete for current mon
            #ifndef POKEMON_EXPANSION
                gTasks[taskId].data[4] = CreateMonIcon(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0, dexFormId); //Create pokemon sprite
            #endif
            #ifdef POKEMON_EXPANSION
                gTasks[taskId].data[6] = CreateMonIcon(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0, dexFormId); //Create pokemon sprite
            #endif
...
static bool8 CalculateMoves(void)
{
    ...
    u16 species = NationalPokedexNumToSpecies(sPokedexListItem->dexNum);
	u16 formSpeciesId = GetFormSpeciesId(species, dexFormId);
    u8 numEggMoves = GetEggMovesSpecies(formSpeciesId, sStatsMovesEgg);
    u8 numLevelUpMoves = GetLevelUpMovesBySpecies(species, sStatsMovesLevelUp, dexFormId);
    u8 numTMHMMoves;
    u8 numTutorMoves = 0;
    u16 movestotal;
    u8 i,j;
    ...
    for (i=0; i<TUTOR_MOVE_COUNT; i++)
    {
        if (CanLearnTutorMove(formSpeciesId, i)) //if (sTutorLearnsets[species] & (1 << i))
        {
    ...
    {
        if(CanSpeciesLearnTMHM(formSpeciesId, j))
        {
...
static void PrintStatsScreen_Left(u8 taskId)
{
    ...
    u32 align_x;
    u8 total_x = 93;
    u16 species = GetFormSpeciesId(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), dexFormId);
	u16 species2 = NationalPokedexNumToSpecies(sPokedexListItem->dexNum);
    u8 strEV[25];
    u8 strBase[14];
    u8 abilities_x = 101;
    u8 abilities_y = 99;
    ...
    if (gTasks[taskId].data[5] == 0)
    {
		...
		PrintInfoScreenTextSmall(gText_Stats_HP, base_x, base_y + base_y_offset*base_i);
        ConvertIntToDecimalStringN(strBase, gVanillaBaseStats[species].baseHP, STR_CONV_MODE_RIGHT_ALIGN, 3);
        PrintInfoScreenTextSmall(strBase, base_x+base_x_first_row, base_y + base_y_offset*base_i);

        PrintInfoScreenTextSmall(gText_Stats_Speed, base_x+base_x_second_row, base_y + base_y_offset*base_i);
        ConvertIntToDecimalStringN(strBase, gVanillaBaseStats[species].baseSpeed, STR_CONV_MODE_RIGHT_ALIGN, 3);
        PrintInfoScreenTextSmall(strBase, base_x+base_x_offset, base_y + base_y_offset*base_i);

        base_i++;
        PrintInfoScreenTextSmall(gText_Stats_Attack, base_x, base_y + base_y_offset*base_i);
        ConvertIntToDecimalStringN(strBase, gVanillaBaseStats[species].baseAttack, STR_CONV_MODE_RIGHT_ALIGN, 3);
        PrintInfoScreenTextSmall(strBase, base_x+base_x_first_row, base_y + base_y_offset*base_i);

        PrintInfoScreenTextSmall(gText_Stats_SpAttack, base_x+base_x_second_row, base_y + base_y_offset*base_i);
		ConvertIntToDecimalStringN(strBase, gVanillaBaseStats[species].baseSpAttack, STR_CONV_MODE_RIGHT_ALIGN, 3);
        PrintInfoScreenTextSmall(strBase, base_x+base_x_offset, base_y + base_y_offset*base_i);

        base_i++;
        PrintInfoScreenTextSmall(gText_Stats_Defense, base_x, base_y + base_y_offset*base_i);
        ConvertIntToDecimalStringN(strBase, gVanillaBaseStats[species].baseDefense, STR_CONV_MODE_RIGHT_ALIGN, 3);
        PrintInfoScreenTextSmall(strBase, base_x+base_x_first_row, base_y + base_y_offset*base_i);

        PrintInfoScreenTextSmall(gText_Stats_SpDefense, base_x+base_x_second_row, base_y + base_y_offset*base_i);
		ConvertIntToDecimalStringN(strBase, gVanillaBaseStats[species].baseSpDefense, STR_CONV_MODE_RIGHT_ALIGN, 3);
        PrintInfoScreenTextSmall(strBase, base_x+base_x_offset, base_y + base_y_offset*base_i);
        base_i++;
		}
    ...
    {    
        if(!FlagGet(FLAG_VANILLA_MODE)){
			...
			PrintInfoScreenTextSmallWhite(gText_Pokedex_Ability0, abilities_x, abilities_y);
			PrintInfoScreenTextSmall(gAbilityNames[ability0], abilities_x, abilities_y + 14);

			if (gBaseStats[species].abilities[1] != ABILITY_NONE)
			{
				PrintInfoScreenTextSmallWhite(gText_Pokedex_Ability1, abilities_x, abilities_y + 30);
				PrintInfoScreenTextSmall(gAbilityNames[gBaseStats[species].abilities[1]], abilities_x, abilities_y + 44);
			}
		}else{
			ability0 = gVanillaBaseStats[species].abilities[0];
			...
			if (gVanillaBaseStats[species].abilities[1] != ABILITY_NONE)
			{
				PrintInfoScreenTextSmallWhite(gText_Pokedex_Ability1, abilities_x, abilities_y + 30);
				PrintInfoScreenTextSmall(gAbilityNames[gVanillaBaseStats[species].abilities[1]], abilities_x, abilities_y + 44);
			}
		}
    }
    else //Hidden abilities
    {
        if(!FlagGet(FLAG_VANILLA_MODE)){
			...
			{
				PrintInfoScreenTextSmallWhite(gText_Pokedex_Ability2, abilities_x, abilities_y);
				PrintInfoScreenTextSmall(gAbilityNames[gBaseStats[species].abilityHidden], abilities_x, abilities_y + 14);
			}
		}else{
			if (gVanillaBaseStats[species].abilityHidden != ABILITY_NONE)
			{
				PrintInfoScreenTextSmallWhite(gText_Pokedex_Ability2, abilities_x, abilities_y);
				PrintInfoScreenTextSmall(gAbilityNames[gVanillaBaseStats[species].abilityHidden], abilities_x, abilities_y + 14);
			}
		}
    }
...
static void Task_LoadEvolutionScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
			//formSpeciesId = GetFormIdFromFormSpeciesId(sPokedexListItem->dexNum);
			formSpeciesId = dexFormId;
			sPokedexView->selectedScreen = EVO_SCREEN;
            //Icon
            FreeMonIconPalettes(); //Free space for new pallete
...
static void handleTargetSpeciesPrint(u8 taskId, u16 targetSpecies, u8 base_x, u8 base_y, u8 base_offset, u8 base_i, u8 formId)
{
...
static u8 PrintEvolutionTargetSpeciesAndMethod(u8 taskId, u16 species, u8 depth, u8 depth_i)
{
	...
	species = GetFormSpeciesId(species, dexFormId);
    ...
    for (i = 0; i < times; i++)
    {
        ...
        case EVO_FRIENDSHIP:
            ...
            handleTargetSpeciesPrint(taskId, targetSpecies, base_x + depth_x*depth, base_y, base_y_offset, base_i, GetFormIdFromFormSpeciesId(targetSpecies)); //evolution mon name
		...
		case EVO_MELMETAL:
            ...
            ConvertIntToDecimalStringN(gStringVar2, gEvolutionTable[species][i].param, STR_CONV_MODE_LEADING_ZEROS, EVO_SCREEN_LVL_DIGITS); //level
            handleTargetSpeciesPrint(taskId, targetSpecies, base_x + depth_x*depth, base_y, base_y_offset, base_i, GetFormIdFromFormSpeciesId(targetSpecies)); //evolution mon name
            StringExpandPlaceholders(gStringVar4, gText_EVO_MELMETAL );
            PrintInfoScreenTextSmall(gStringVar4, base_x + depth_x*depth+base_x_offset, base_y + base_y_offset*base_i);
            break;
    ...
    if (dexMon->owned) // Show filed bars
    {
        ...
        for (i = 0; i < NUM_STATS; i++)
        {
            if(!FlagGet(FLAG_VANILLA_MODE))
				...
				statValue = *((u8*)(&gVanillaBaseStats[species]) + sBaseStatOffsets[i]);
...
static void Task_LoadFormsScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            gTasks[taskId].data[4] = CreateMonIcon(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0, 0); //Create pokemon sprite
            gSprites[gTasks[taskId].data[4]].oam.priority = 0;
        }
        EvoFormsPage_PrintAToggleUpdownEvos(); //HGSS_Ui Navigation buttons
        gMain.state++;
...
static void Task_HandleFormsScreenInput(u8 taskId)
{
	u16 species = sPokedexListItem->dexNum;
	u8  numForms = 0;
...
static void PrintForms(u8 taskId, u16 species)
{
	...
	const u8 gText_FORMS_CHANGE[] = _("Press L or R to change between Forms.");
    ...
    for (i=0; i < 30; i++)
    {
        ...
        else
        {
			...
			if(dexFormId == i)//Selection Sprite asdf
				LoadMonIconPalette(SPECIES_NONE);
			
            if (times < 7)
                gTasks[taskId].data[4+times] = CreateMonIcon(species, SpriteCB_MonIcon, 52 + 34*(times-1), 31, 4, 0, i); //Create pokemon sprite
            else if (times < 14)
                gTasks[taskId].data[4+times] = CreateMonIcon(species, SpriteCB_MonIcon, 18 + 34*(times-7), 70 - y_offset_icons, 4, 0, i); //Create pokemon sprite
            else if (times < 21)
                gTasks[taskId].data[4+times] = CreateMonIcon(species, SpriteCB_MonIcon, 18 + 34*(times-14), 104 - y_offset_icons, 4, 0, i); //Create pokemon sprite
            else
                gTasks[taskId].data[4+times] = CreateMonIcon(species, SpriteCB_MonIcon, 18 + 34*(times-21), 138 - y_offset_icons, 4, 0, i); //Create pokemon sprite
    ...
    if (times == 0)
    {
        ...
        PrintInfoScreenTextSmall(gText_FORMS_CHANGE, base_x, base_y + base_y_offset*times);
    }
}
static void Task_SwitchScreensFromFormsScreen(u8 taskId)
{

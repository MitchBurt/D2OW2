...
void CB2_InitTitleScreen(void)
{
    ...
    case 1:
        ...
        LoadCompressedSpriteSheet(&sSpriteSheet_EmeraldVersion[0]);									//Not Changed
        LoadCompressedSpriteSheet(&sSpriteSheet_PressStart[0]);										//Not Changed
        LoadCompressedSpriteSheet(&sPokemonLogoShineSpriteSheet[0]);								//Not Changed
        LoadPalette(gTitleScreenEmeraldVersionPal, 0x100, 0x20);									//Not Changed
        LoadSpritePalette(&sSpritePalette_PressStart[0]);											//Not Changed
    ...
    case 4:
		...
		if (gSaveBlock2Ptr->optionsDifficulty == OPTIONS_MUSIC_KANTO)
			...
			m4aSongNumStart(PL_SEQ_TITLE01);
		else 
...
void CB2_GoToSoundCheckScreen(void)
{
    if (!UpdatePaletteFade())

...
static void DrawPocketIndicator(void);
...
void ItemMenuTaskScrollBackgroundToss(u8 taskId);
...
static const struct MenuAction sItemMenuActions[] = {
    ...
    [ITEMMENUACTION_BY_MOVE_TYPE] = {sMenuText_ByType, ItemMenu_SortByMoveType},
...
{
    GoToBagMenu(ITEMMENULOCATION_FIELD, POCKETS_COUNT, CB2_ReturnToUIMenu); //CB2_ReturnToFieldWithOpenMenu
}
...
bool8 SetupBagMenu(void)
{
	...
	VarSet(VAR_CURRENT_BAG_POCKET, gBagPositionStruct.pocket);
    ...
    case 13:
        ...
        break;
    case 14:
        //CreateTask(ItemMenuTaskScrollBackgroundToss, 0);
...
void BagMenu_InitBGs(void)
{
    ...
    InitBgsFromTemplates(0, sBgTemplates_ItemMenu, 4);//Everything
    SetBgTilemapBuffer(2, gBagMenu->tilemapBuffer[BAG_MENU_BG_NORMAL]);//whole BG
    SetBgTilemapBuffer(SCROLLING_BACKGROUND_NUM, gBagMenu->tilemapBuffer[BAG_MENU_BG_SCROLLING]);
...
const u32 sBagMenuScrollingBGTilemap[] = INCBIN_U32("graphics/ui_menu/bag_menu/scrolling_bg.bin.lz");
...
void PrintItemSoldAmount(int windowId, int numSold, int moneyEarned)
{
    ...
    PrintMoneyAmount(windowId, 38, 1, moneyEarned*10, 0);
}
...
static void SwitchBagPocket(u8 taskId, s16 deltaBagPocketId, u16 a3)
{
    s16* data = gTasks[taskId].data;
    u8 pocketId;
	u8 CurrentPocket = gBagPositionStruct.pocket;
    data[13] = 0;
    data[12] = 0;
    ...
    VarSet(VAR_CURRENT_BAG_POCKET, pocketId);
	DrawPocketIndicator();
    ScheduleBgCopyTilemapToVram(2);
...
static const u8 sHeldItemIcon[]   = INCBIN_U8("graphics/ui_menu/bag_menu/held_item_icon.4bpp");
static const u8 sMedicineIcon[]   = INCBIN_U8("graphics/ui_menu/bag_menu/medicine_icon.4bpp");
static const u8 sPokeballIcon[]   = INCBIN_U8("graphics/ui_menu/bag_menu/pokeball_icon.4bpp");
static const u8 sBattleItemIcon[] = INCBIN_U8("graphics/ui_menu/bag_menu/battle_item_icon.4bpp");
static const u8 sTypeItemIcon[]   = INCBIN_U8("graphics/ui_menu/bag_menu/type_item.4bpp");
static const u8 sMegaStoneIcon[]  = INCBIN_U8("graphics/ui_menu/bag_menu/mega_stone_icon.4bpp");
static const u8 sBerryIcon[]      = INCBIN_U8("graphics/ui_menu/bag_menu/berry_icon.4bpp");
...
static void DrawPocketIndicator()
{
	...
	u8 CurrentPocket = VarGet(VAR_CURRENT_BAG_POCKET);
	u8 i;
	u8 windowId = 2;
	u8 posX = CurrentPocket;
	u8 posY = 2;
	
	FillWindowPixelBuffer(windowId, PIXEL_FILL(TEXT_COLOR_TRANSPARENT));
	
	if(CurrentPocket >= 8){
		posX = (CurrentPocket - 8);
		posY++;
	}
	
	switch(CurrentPocket){
	case ITEMS_POCKET:
		BlitBitmapToWindow(windowId, sHeldItemIcon, (posX*8), (posY*8), 8, 8);
		break;
	case MEDICINE_POCKET:
		BlitBitmapToWindow(windowId, sMedicineIcon, (posX*8), (posY*8), 8, 8);
		break;
	case BALLS_POCKET:
		BlitBitmapToWindow(windowId, sPokeballIcon, (posX*8), (posY*8), 8, 8);
		break;
	case BATTLEITEMS_POCKET:
		BlitBitmapToWindow(windowId, sBattleItemIcon, (posX*8), (posY*8), 8, 8);
		break;
	case TYPEITEMS_POCKET:
		BlitBitmapToWindow(windowId, sTypeItemIcon, (posX*8), (posY*8), 8, 8);
		break;
	case MEGASTONES_POCKET:
		BlitBitmapToWindow(windowId, sMegaStoneIcon, (posX*8), (posY*8), 8, 8);
		break;
	case BERRIES_POCKET:
		BlitBitmapToWindow(windowId, sBerryIcon, (posX*8), (posY*8), 8, 8);
		break;
	case POWERUP_POCKET:
		BlitBitmapToWindow(windowId, sPowerUpIcon, (posX*8), (posY*8), 8, 8);
		break;
	case TMHM_POCKET:
		BlitBitmapToWindow(windowId, sTMIcon, (posX*8), (posY*8), 8, 8);
		break;
	case KEYITEMS_POCKET:
		BlitBitmapToWindow(windowId, sKeyItemIcon, (posX*8), (posY*8), 8, 8);
		break;
    case CHARMS_POCKET:
		BlitBitmapToWindow(windowId, sCharmsIcon, (posX*8), (posY*8), 8, 8);
		break;
	}
...
static void BagMenu_TryDepositItem(u8 taskId)
{
    ...
    if (ItemId_GetImportance(gSpecialVar_ItemId))
    {
        ...
        gTasks[taskId].func = BagMenu_Deposit_WaitForABPress;
			...
			gSpecialVar_ItemId == ITEM_NUGGET      		||
			gSpecialVar_ItemId == ITEM_ABILITY_CAPSULE	||
			gSpecialVar_ItemId == ITEM_ABILITY_PATCH	||
			gSpecialVar_ItemId == ITEM_EXIOLITE			||
		   (gSpecialVar_ItemId >= ITEM_BUG_MEMORY && 
		    gSpecialVar_ItemId <= ITEM_WATER_MEMORY) 	||
		   (gSpecialVar_ItemId >= ITEM_GIGALPHITE && 
		    gSpecialVar_ItemId <= ITEM_YGGDRACONITE))
    {
        BagMenu_Print(1, 1, gText_CantStoreImportantItems, 3, 1, 0, 0, 0, 0);
        gTasks[taskId].func = BagMenu_Deposit_WaitForABPress;
    }
...
static const u16 sItemsByType[ITEMS_COUNT] =
{
	...
	[ITEM_POWER_CANDY] = ITEM_TYPE_STAT_BOOST_DRINK,
    ...
    [ITEM_COSMIC_STONE] = ITEM_TYPE_EVOLUTION_STONE,
    [ITEM_METAL_COAT] = ITEM_TYPE_EVOLUTION_STONE,
    [ITEM_UP_GRADE] = ITEM_TYPE_EVOLUTION_STONE,
	...
	[ITEM_EXIOLITE] = ITEM_TYPE_HELD_ITEM,
    ...
    [ITEM_YGGDRACONITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SKORPYROSITE_X] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SKORPYROSITE_Y] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SPERIATHONITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_PARANGELITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_FAZEAGLEITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_ALAKAZITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SHEMUENITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_BIPHARMITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_TROLOLOITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_ITRIMIDITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SNEDDITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GOTHOTITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GODNITE_X] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GODNITE_Y] = ITEM_TYPE_MEGA_STONE,
    [ITEM_BACIDITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_TOMITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_NUGGIEITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_HERACRONITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GAHNOMEDITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_FROSILISKITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_INCREDALENITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GOURMASTERITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_BAITANKITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_AWMANITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SABLENITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_MAWILITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_VOIDITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_LONKESTITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_MANECTITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SHARKEGNITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_ROCKITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_UNSCNITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_BANETTITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GOALEMITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_JANKITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SALAMENCITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_BUGITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_STINGWELLITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_DUSHROOMITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_LOPUNNITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_TRUCKOISEITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_BLOKNOCKNITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GIGALPHITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_GALLADITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_UNJOYITE] = ITEM_TYPE_MEGA_STONE,
    [ITEM_SHEPEIRDITE] = ITEM_TYPE_MEGA_STONE,
    ...
    [ITEM_THEVOIDNIUM_Z] = ITEM_TYPE_Z_CRYSTAL,
    ...
    */
    ...
    [ITEM_HORN_FOSSIL] = ITEM_TYPE_FOSSIL,
    [ITEM_JAW_FOSSIL] = ITEM_TYPE_FOSSIL,
    [ITEM_TOOTH_FOSSIL] = ITEM_TYPE_FOSSIL,
...
static void ItemMenu_SortByMoveType(u8 taskId)
{
    gTasks[taskId].tSortType = SORT_BY_MOVE_TYPE; //by itemID
...
void SortItemsInBag(u8 pocket, u8 type)
{
    ...
    case TYPEITEMS_POCKET:
        ...
        itemAmount = BAG_TYPEITEMS_COUNT;
        break;
    case MEGASTONES_POCKET:
...
static s8 CompareItemsByMoveType(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2)
{
    ...
    return CompareItemsById(itemSlot1, itemSlot2); //Items with the same type are sorted by number
}
...
bool8 UseRegisteredKeyItemOnField(u8 button)
{
    ...
    if (InUnionRoom() == TRUE || InBattlePyramid() || InBattlePike() || InMultiPartnerRoom() == TRUE || gPlayerAvatar.preventStep)
        return FALSE;

...
static void Cmd_setplaindamage(void);
static void Cmd_jumpifsignaturemove(void);
...
{
    FLAG_RECEIVED_BADGE_01,
	FLAG_RECEIVED_BADGE_02,
	FLAG_RECEIVED_BADGE_03,
	FLAG_RECEIVED_BADGE_04,
	FLAG_RECEIVED_BADGE_05,
...
void (* const gBattleScriptingCommandsTable[])(void) =
{
    ...
    Cmd_metronome,                              // 0x9E
    Cmd_setplaindamage,                         // 0x9F
    Cmd_jumpifsignaturemove,                    // 0xA0
...
static bool32 TryAegiFormChange(void)
{
    ...
    if ((GetBattlerAbility(gBattlerAttacker) != ABILITY_STANCE_CHANGE
		...
		&& GetBattlerAbility(gBattlerAttacker) != ABILITY_FLOWER_GIFT)
...
{
    u32 calc, moveAcc, newmoveAcc, signatureAcc, atkHoldEffect, atkParam, defHoldEffect, defParam, atkAbility, defAbility;
    s8 buff, accStage, evasionStage;
    bool8 accuracyChanged = FALSE;
    ...
    moveAcc = getMoveAccuracy(move, GetFormSpeciesId(gBattleMons[battlerAtk].species, gBattleMons[battlerAtk].formId));
    ...
    if (atkAbility == ABILITY_HUSTLE && IS_BATTLER_MOVE_PHYSICAL(move, battlerAtk))
...
s32 CalcCritChanceStage(u8 battlerAtk, u8 battlerDef, u32 move, bool32 recordAbility)
{
    ...
    else
    {
        ...
        critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                    ...
                    + 2 * (holdEffectAtk == HOLD_EFFECT_LUCKY_PUNCH && gBattleMons[gBattlerAttacker].species == SPECIES_ARACKNE)
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else if (!DoesDisguiseBlockMove(gBattlerAttacker, gActiveBattler, gCurrentMove))
        {
			...
			if (!(gHitMarker & HITMARKER_IGNORE_SUBSTITUTE) &&
				...
				gBattleMoveDamage > 0)
			{
				VarSet(VAR_LAST_DAMAGE_DONE, gBattleMoveDamage);
				
				if(gSaveBlock2Ptr->optionsDamageDoneMessageEnabled == 0){
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                else
                {
					...
					moveDamage = gBattleMoveDamage;
                }

                if (!gSpecialStatuses[gActiveBattler].dmg && !(gHitMarker & HITMARKER_x100000))
                    gSpecialStatuses[gActiveBattler].dmg = gHpDealt;

                if (IS_BATTLER_MOVE_PHYSICAL(gCurrentMove, gActiveBattler) && !(gHitMarker & HITMARKER_x100000) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (!IS_BATTLER_MOVE_PHYSICAL(gCurrentMove, gActiveBattler) && !(gHitMarker & HITMARKER_x100000))
                {
...
{
    u32 stringId = 0;
	u16 lastMoveDamage = VarGet(VAR_LAST_DAMAGE_DONE);
...
void SetMoveEffect(bool32 primary, u32 certain)
{
     ...
     if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT || gBattleScripting.moveEffect >= MOVE_EFFECT_INTOXICATE) // status change
    {
        ...
        case STATUS1_TOXIC_POISON:
            if (!CanBePoisoned(gBattleScripting.battler, gEffectBattler) && (primary == TRUE || certain == MOVE_EFFECT_CERTAIN))
            {
...
static void Cmd_seteffectwithchance(void)
{
    ...
    u8 FormId = gBattleMons[gBattlerAttacker].formId;
    u16 speciesId = GetFormSpeciesId(species, FormId);
    u32 percentChance = getMoveSecondaryEffectChance(gCurrentMove, speciesId);
    ...
    case 0: // check if should receive exp at all
        ...
        else
        {
			if(gSaveBlock2Ptr->expShare)
				PrepareStringBattle(STRINGID_ALL_PKMNGAINEDEXP, gBattleStruct->expGetterBattlerId);
                ...
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
					...
					for (count = CalculatePlayerPartyCount(), i = 0; i < count; i++)
					{
						if (!GetMonData(&gPlayerParty[i], MON_DATA_SANITY_IS_EGG))
						{
							...
							if (ability == ABILITY_LUCKY_BOOST){
								luckyboost = luckyboost + 20;
							}
						}
					}
                    ...
                    if(FlagGet(FLAG_NEW_GAME_PLUS) && VarGet(VAR_NEW_GAME_PLUS_COUNT) != 0){
					...
					if(gBattleMoveDamage > MAX_POSSIBLE_EXP)
                        gBattleMoveDamage = MAX_POSSIBLE_EXP;
...
static void Cmd_switchineffects(void)
{
    ...
    if (!(gSideStatuses[GetBattlerSide(gActiveBattler)] & SIDE_STATUS_SPIKES_DAMAGED)
        ...
        && GetBattlerAbility(gActiveBattler) != ABILITY_MOUNTAINEER
...
static void PutMonIconOnLvlUpBox(void)
{
    ...
    const u8* iconPtr = GetMonIconPtr(species, personality, formId);
...
{
    u16 formSpeciesId = GetFormSpeciesId(gBattleMons[battler].species, gBattleMons[battler].formId);
    CalculateMonStats(mon);
...
static void Cmd_various(void)
{
	...
	case VARIOUS_GIVE_DROPPED_ITEMS:
		...
		//gLastUsedItem = gBattleStruct->usedHeldItems[gActiveBattler];
         {
        u8 i;
        ...
        {
            gLastUsedItem = gBattleStruct->usedHeldItems[battlers[i]];
            gBattleStruct->usedHeldItems[battlers[i]] = ITEM_NONE;
...
static void Cmd_jumpifsignaturemove(void)
{
    ...
    u16 battlerspecies = GetFormSpeciesId(gBattleMons[battlerId].species, gBattleMons[battlerId].formId);
    bool32 isSignatureMove = FALSE;
    u16 species = T2_READ_16(gBattlescriptCurrInstr + 2);
    u16 move = T2_READ_16(gBattlescriptCurrInstr + 4);

    if(species == battlerspecies && move == gCurrentMove)
...
static void Cmd_handleballthrow(void)
{
	...
	else if (FlagGet(FLAG_TOTEM_BATTLE) && (gBattleMons[gBattlerTarget].hp >= gBattleMons[gBattlerTarget].maxHP / 3))
    {
        BtlController_EmitBallThrowAnim(0, BALL_TRAINER_BLOCK);
        MarkBattlerForControllerExec(gActiveBattler);
...
static void Cmd_givecaughtmon(void)
{
    ...
    switch (speciesID)
    {
        ...
        case SPECIES_STINGWELL_MEGA:
        case SPECIES_TROJOLIN_MEGA:
        ...
        case SPECIES_AEGISLASH_BLADE:
            newSpeciesID = SPECIES_AEGISLASH;
            SetMonData(&gEnemyParty[gBattlerPartyIndexes[GetCatchingBattler()]], MON_DATA_FORM_ID, &newFormId);
            SetMonData(&gEnemyParty[gBattlerPartyIndexes[GetCatchingBattler()]], MON_DATA_SPECIES, &newSpeciesID);
        break;
        case SPECIES_CRAMORANT_GULPING:
        case SPECIES_CRAMORANT_GORGING:
            newSpeciesID = SPECIES_CRAMORANT;
            SetMonData(&gEnemyParty[gBattlerPartyIndexes[GetCatchingBattler()]], MON_DATA_FORM_ID, &newFormId);
            SetMonData(&gEnemyParty[gBattlerPartyIndexes[GetCatchingBattler()]], MON_DATA_SPECIES, &newSpeciesID);
        break;
    }
...
static void Cmd_settelekinesis(void)
{
    if (gStatuses3[gBattlerTarget] & (STATUS3_TELEKINESIS | STATUS3_ROOTED | STATUS3_SMACKED_DOWN)
        || gFieldStatuses & STATUS_FIELD_GRAVITY
        || (gBattleMons[gBattlerTarget].species == SPECIES_DIGLETT || gBattleMons[gBattlerTarget].species == SPECIES_GHOMA))
    {
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
    else
...
bool32 TryResetAllBattlersStatChanges(void)
{
    u32 j, i;
    bool32 ret = FALSE;
    u8 numBattlers = MAX_BATTLERS_COUNT / 2;

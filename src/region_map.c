...
static void CreateKantoFlyDestIcons(void);//Kanto
...
//Kanto
static const u16 sRegionMapKantoBg_Pal[] 		= INCBIN_U16("graphics/pokenav/region_map.gbapal");
static const u32 sRegionMapKantoBg_GfxLZ[] 		= INCBIN_U32("graphics/pokenav/region_map_kanto/region_map_kanto.8bpp.lz");
static const u32 sRegionMapKantoBg_TilemapLZ[] 	= INCBIN_U32("graphics/pokenav/region_map_kanto/region_map_kanto.bin.lz");
...
static const u16 sRegionMapPlayerIcon_RedPal[] 			= INCBIN_U16("graphics/pokenav/region_map_kanto/player_icon_red.gbapal");
static const u8 sRegionMapPlayerIcon_RedGfx[] 			= INCBIN_U8("graphics/pokenav/region_map_kanto/player_icon_red.4bpp");
static const u16 sRegionMapPlayerIcon_LeafPal[] 		= INCBIN_U16("graphics/pokenav/region_map_kanto/player_icon_leaf.gbapal");
static const u8 sRegionMapPlayerIcon_LeafGfx[] 			= INCBIN_U8("graphics/pokenav/region_map_kanto/player_icon_leaf.4bpp");


static const u16 sRegionMapPlayerIcon_HilbertPal[] 		= INCBIN_U16("graphics/pokenav/hilbert_icon.gbapal");
static const u8 sRegionMapPlayerIcon_HilbertGfx[] 		= INCBIN_U8("graphics/pokenav/hilbert_icon.4bpp");

static const u16 sRegionMapPlayerIcon_BrendanRSPal[] 	= INCBIN_U16("graphics/pokenav/brendan_rs_icon.gbapal");
static const u8 sRegionMapPlayerIcon_BrendanRSGfx[] 	= INCBIN_U8("graphics/pokenav/brendan_rs_icon.4bpp");
static const u16 sRegionMapPlayerIcon_MayRSPal[] 		= INCBIN_U16("graphics/pokenav/may_rs_icon.gbapal");
static const u8 sRegionMapPlayerIcon_MayRSGfx[] 		= INCBIN_U8("graphics/pokenav/may_rs_icon.4bpp");
...
void InitRegionMap(struct RegionMap *regionMap, bool8 zoomed)
{
	...
	else if(gMapHeader.region == REGION_SEVII || gMapHeader.region == REGION_KANTO){
...
bool8 LoadKantoRegionMapGfx(void)
{
    ...
    case 0:
        if (gRegionMap->bgManaged)
            DecompressAndCopyTileDataToVram(gRegionMap->bgNum, sRegionMapKantoBg_GfxLZ, 0, 0, 0);
        else
    ...
    case 1:
        ...
        {
			if (!FreeTempTileDataBuffersIfPossible())
                DecompressAndCopyTileDataToVram(gRegionMap->bgNum, sRegionMapKantoBg_TilemapLZ, 0, 0, 1);
        }
        else
        {
            //LZ77UnCompVram(sRegionMapBg_TilemapLZ, (u16 *)BG_SCREEN_ADDR(28));
...
static void InitMapBasedOnPlayerLocation(void) {
	if(gMapHeader.region == REGION_HOENN)
		...
		KantoRegionMap_GetSectionCoordsFromCurrFieldPos(&gRegionMap->mapSecId, &gRegionMap->cursorPosX, &gRegionMap->cursorPosY, &gRegionMap->playerIsInCave);
}
...
static void CB_HandleFlyMapInput(void)
{
    if (sFlyMap->state == 0)
    {
        ...
        case MAP_INPUT_A_BUTTON:
            ...
            {
                m4aSongNumStart(RegionalMusicHandler(SE_SELECT));
        ...
        if (!UpdatePaletteFade())
        {
            ...
            if (sFlyMap->choseFlyLocation)
            {
				...
				case MAPSEC_PEWTER_CITY:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_PEWTER_CITY);
                    break;
				case MAPSEC_CERULEAN_CITY:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_CERULEAN_CITY);
                    break;
				case MAPSEC_SAFFRON_CITY:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_SAFFRON_CITY);
                    break;
				case MAPSEC_VERMILION_CITY:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_VERMILION_CITY);
                    break;
				case MAPSEC_LAVENDER_TOWN:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_LAVENDER_TOWN);
                    break;
				case MAPSEC_CELADON_CITY:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_CELADON_CITY);
                    break;
				...
				case MAPSEC_POKEMON_LEAGUE:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_LITTLEROOT_TOWN_BRENDANS_HOUSE);
                    break;
				case MAPSEC_ONE_ISLAND:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_ONE_ISLAND);
                    break;
				case MAPSEC_TWO_ISLAND:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_TWO_ISLAND);
                    break;
				case MAPSEC_THREE_ISLAND:
					SetWarpDestinationToHealLocation(HEAL_LOCATION_THREE_ISLAND);
                    break;
                default:

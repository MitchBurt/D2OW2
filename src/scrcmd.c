...
bool8 ScrCmd_addscaleditem(struct ScriptContext *ctx)
{
    u16 itemId = VarGet(ScriptReadHalfword(ctx));
    u32 quantity = VarGet(ScriptReadHalfword(ctx));
	itemId = GetScaledItem(itemId);
...
bool8 ScrFunc_bufferlivemonspeciesname(struct ScriptContext *ctx)
{
    ...
    u8 *dest = sScriptStringVars[stringVarIndex];
    u32 species = GetMonData(&gPlayerParty[POF_GetFollowerSlotId()], MON_DATA_SPECIES);
    StringCopy(dest, gSpeciesNames[species]);
    return FALSE;
}
...
{
  u16 species = GetMonData(&gPlayerParty[POF_GetFollowerSlotId()], MON_DATA_SPECIES);
  PlayCry5(species, 0);
  return FALSE;
}
...
bool8 ScrFunc_getfolloweraction(struct ScriptContext *ctx) // Essentially a big switch for follower messages
{
	  ...
	  struct Pokemon *mon = &gPlayerParty[POF_GetFollowerSlotId()];
	  if (mon == NULL) {
	  ...
	  // 6. Ability-based messages
	  if(ability == ABILITY_PICKUP && rand <= 6)
		  message_choices[n_choices++] = EventScript_FollowerPickup;
	  else if(ability == ABILITY_PICKUP && rand == 10)
...
bool8 ScrCmd_canFollowerFly(struct ScriptContext *ctx)
{
    u8 i;
	
	gSpecialVar_Result = PARTY_SIZE;
	if(CanMonLearnTMHM(&gPlayerParty[POF_GetFollowerSlotId()], ITEM_TM76_FLY - ITEM_TM01_FOCUS_PUNCH))
	{	
		gSpecialVar_Result = 7;
		gSpecialVar_0x8004 = GetMonData(&gPlayerParty[POF_GetFollowerSlotId()], MON_DATA_SPECIES, NULL);;
	}
    return FALSE;
}
...
bool8 ScrCmd_setwildbattlewithcustommoves(struct ScriptContext *ctx)
{
    ...
    u16 m4 = ScriptReadHalfword(ctx);
    u16 m5 = ScriptReadHalfword(ctx);
	...
	if (m4 != 3) {
		SetMonData(pkmn, MON_DATA_ABILITY_NUM, &m4);
    }      
	if (m5 != 0) {
		SetMonData(pkmn, MON_DATA_HIDDEN_NATURE, &m5);
    }
...
bool8 ScrCmd_showshinymonpic(struct ScriptContext *ctx)
{
    u16 species = VarGet(ScriptReadHalfword(ctx));
    u8 x = ScriptReadByte(ctx);
    u8 y = ScriptReadByte(ctx);
...
bool8 ScrCmd_checkcompanionstuff(struct ScriptContext *ctx)
{
    u16 flag  = ScriptReadHalfword(ctx);
    switch(flag){
        case COMPANION_TRAINER_PASSWORD:
            gSpecialVar_Result = CreatePlayerPartyPassword();
        break;
        case COMPANION_WORKING_PASSWORD_TRAINER:
            gSpecialVar_Result = CheckRamForPasswordTrainer();
        break;
        case COMPANION_WORKING_ROAMER:
            gSpecialVar_Result = (&gSaveBlock1Ptr->roamer)->species != SPECIES_NONE;
        break;
    }
    return FALSE;
}
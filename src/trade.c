...
static void CB2_ReturnToTradeMenu(void)
{
    ...
    case 7:
        ...
        for (i = 0; i < sTradeMenuData->partyCounts[TRADE_PLAYER]; i++)
        {
			...
			SetMonData(mon, MON_DATA_HELD_ITEM, &defaultHeldItem);
        }
...
int GetUnionRoomTradeMessageId(struct GFtgtGnameSub rfuPlayer, struct GFtgtGnameSub rfuPartner, u16 playerSpecies2, u16 partnerSpecies, u8 requestedType, u16 playerSpecies, u8 isObedientBitSet)
{
	...
	for(i = 0; i < ARRAY_COUNT(LegendaryPokemon); i++){
		if (playerSpecies2 == LegendaryPokemon[i])
			return UR_TRADE_MSG_PARTNERS_MON_CANT_BE_TRADED;
	}
...
static void LoadTradeMonPic(u8 whichParty, u8 state)
{
    ...
    case 0:
        ...
        HandleLoadSpecialPokePic(&gMonFrontPicTable[formSpeciesId], gMonSpritesGfxPtr->sprites[whichParty * 2 + 1], formSpeciesId, personality);
    ...
    case 1:
        formId = GetMonData(mon, MON_DATA_FORM_ID);
        SetMultiuseSpriteTemplateToPokemon(GetMonSpritePalStruct(mon)->tag, pos, formId);
...
static bool8 AnimateTradeSequenceCable(void)
{
    ...
    case 65:
        ...
        {
            HandleLoadSpecialPokePic(&gMonFrontPicTable[GetFormSpeciesId(sTradeData->monSpecies[TRADE_PARTNER], sTradeData->monFormIds[TRADE_PARTNER])], gMonSpritesGfxPtr->sprites[3], GetFormSpeciesId(sTradeData->monSpecies[TRADE_PARTNER], sTradeData->monFormIds[TRADE_PARTNER]), sTradeData->monPersonalities[TRADE_PARTNER]);
            sTradeData->state++;
        }
        break;
    case 66:
    ...
    case 72: // Only if in-game trade
        ...
        evoTarget = GetEvolutionTargetSpecies(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], TRUE, ITEM_NONE, GetMonData(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PARTNER]], MON_DATA_SPECIES), &formIdTarget);
        if (evoTarget != SPECIES_NONE)
        {
            TradeEvolutionScene(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], evoTarget, sTradeData->pokePicSpriteIdxs[TRADE_PARTNER], gSelectedTradeMonPositions[TRADE_PLAYER], formIdTarget);
        }
        sTradeData->state++;
        break;
    case 73:
...
static void _CreateInGameTradePokemon(u8 whichPlayerMon, u8 whichInGameTrade)
{
	...
	CreateMon(pokemon, inGameTrade->species, level, 32, FALSE, 0, OT_ID_PRESET, inGameTrade->otId, GetFormIdFromFormSpeciesId(inGameTrade->species));
...
static u8 WonderTradeGetAbilityNum(u16 species, u8 dexseen)
{
    ...
    if (genAbility && gBaseStats[species].abilityHidden != ABILITY_NONE)
    {
        abilityNum = 2;
    }
    else
...
u16 WonderTradeCreatePokeball(u16 species)
{
	...
	if ((gBaseStats[species].type1 == TYPE_DARK && random <= 2)
     || (gBaseStats[species].type2 == TYPE_DARK && random <= 4 && random > 2))
	return ITEM_MOON_BALL;
	else if ((gBaseStats[species].type1 == TYPE_STEEL && random <= 2)
     ||      (gBaseStats[species].type2 == TYPE_STEEL && random <= 4 && random > 2))
	return ITEM_HEAVY_BALL;
	else if ((gBaseStats[species].type1 == TYPE_BUG && random <= 2)
     ||      (gBaseStats[species].type2 == TYPE_BUG && random <= 4 && random > 2))
	return ITEM_NET_BALL;
	else if (gBaseStats[species].bodyColor == BODY_COLOR_BLUE && random <= 6)
	return ITEM_DIVE_BALL;
	else if (gBaseStats[species].bodyColor == BODY_COLOR_PINK && random <= 6)
	return ITEM_LOVE_BALL;
	else if (gBaseStats[species].bodyColor == BODY_COLOR_PINK && random <= 6)
	return ITEM_LOVE_BALL;
	else if (gBaseStats[species].bodyColor == BODY_COLOR_GREEN && random <= 6)
	return ITEM_FRIEND_BALL;
	else if (gBaseStats[species].abilities[0] == ABILITY_THICK_FAT && random <= 6)
	return ITEM_HEAVY_BALL;
	else if (random == 8)
...
static void _CreateInGameWonderTradePokemon(u8 whichPlayerMon, u8 whichInGameTrade)
{
	...
	u8 i;
    u8 level           = GetMonData(&gPlayerParty[whichPlayerMon], MON_DATA_LEVEL);
	u16 tradeditem     = GetMonData(&gPlayerParty[whichPlayerMon], MON_DATA_HELD_ITEM);
	u16 species        = GetWildPokemon(GetRandomFirstStage(SPECIES_NONE), level, GetMonData(&gPlayerParty[0], MON_DATA_HELD_ITEM));
	u16 trainerNum     = (Random() % 300) + 1;
	u16 CaughtNum      = GetNationalPokedexCount(FLAG_GET_SEEN); //Unused
    u16 SeenNum        = GetNationalPokedexCount(FLAG_GET_CAUGHT);
	u16 speciesId      = getWonderTradePokemonFormID(species);
	u16 Pkmnhelditem   = CreateWonderTradeItem(speciesId, SeenNum);
	u16 Pokeball       = WonderTradeCreatePokeball(speciesId);
	u8  PkmnAbilityNum = WonderTradeGetAbilityNum(speciesId, SeenNum);
	u8  formId         = GetFormIdFromFormSpeciesId(speciesId);
	u16 PkmnMoves[]    = {MOVE_NONE, MOVE_NONE, MOVE_NONE, MOVE_NONE};
	...
	do{
		GetWildPokemon(GetRandomFirstStage(SPECIES_NONE), level, GetMonData(&gPlayerParty[0], MON_DATA_HELD_ITEM));
		getWonderTradePokemonFormID(species);
	}
	while(!IsPokemonValid(species));
	
	WonderTradeCreateMoveset(speciesId, SeenNum, level, PkmnMoves);
    CreateMon(pokemon, species, level, 32, FALSE, 0, OT_ID_PRESET, inGameTrade->otId, formId);

	
	for (i = 0; i <= 12; i++)
		otName[i] = gTrainers[trainerNum].trainerName[i];
	...
	SetMonData(pokemon, MON_DATA_OT_NAME, otName);
    SetMonData(pokemon, MON_DATA_OT_GENDER, &inGameTrade->otGender);
	
	if(tradeditem != ITEM_NONE)
		SetMonData(pokemon, MON_DATA_HELD_ITEM, &Pkmnhelditem);	
	
	if(GetNumBadges() >= 2)
	SetMonData(pokemon, MON_DATA_POKEBALL, &Pokeball);
	
	if(GetNumBadges() >= 4)
		SetMonData(pokemon, MON_DATA_ABILITY_NUM, &PkmnAbilityNum);	

	//if(GetNumBadges() >= 3 && PkmnMoves[0] != MOVE_NONE)
	//	SetMonData(pokemon, MON_DATA_MOVE1, &PkmnMoves[0]);

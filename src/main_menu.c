 ...
 * Task_NewGameBirchSpeech_StartBirchScubitePlatformFade
 * Task_NewGameBirchSpeech_StartBirchScubitePlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchScubite
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForScubite
 * Task_NewGameBirchSpeechSub_WaitForScubite
...
static void Task_NewGameBirchSpeechSub_WaitForScubite(u8);
static void Task_NewGameBirchSpeech_StartBirchScubitePlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchScubite(u8);
...
{
	const u8 gText_EmulationAccuracyTest[] = _("Emulation accuracy test.\nNumber of Failed Tests: {STR_VAR_1}{PAUSE_UNTIL_PRESS}");//1Mサブきばんが ささっていません！
	
	u64 numErrors = 0;//RunAgbAccuracyTests(-1)
    ...
    if (!gPaletteFade.active)
    {
		...
		if(numErrors > 65)
		{
            ConvertIntToDecimalStringN(gStringVar1, numErrors, STR_CONV_MODE_LEFT_ALIGN, 5);
			StringExpandPlaceholders(gStringVar4, gText_EmulationAccuracyTest);
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (gTasks[taskId].tMenuType)
        {
            ...
            case HAS_SAVED_GAME:
                ...
                if(FlagGet(FLAG_SYS_GAME_CLEAR) && !FlagGet(FLAG_FULL_RANDOMIZED_MODE))
                    AddTextPrinterParameterized3(3, 1, 0, 1, sTextColor_Headers, -1, gText_MainMenuNewGamePlus);
                else
            ...
            case HAS_MYSTERY_EVENTS:
                ...
                AddTextPrinterParameterized3(5, 1, 0, 1, sTextColor_Headers, -1, gText_MainMenuSoundTest);
...
#define tScubiteSpriteId data[9]
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInWelcome(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        else
        {
            ...
            AddTextPrinterForBlackMessage(1);
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tScubiteSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, 0x0000FFFF, SPECIES_MEADOSAUR);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForScubite;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForScubite(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tScubiteSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForBlackMessage(1);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchScubitePlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchScubitePlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tScubiteSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tScubiteSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tScubiteSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchScubite;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchScubite(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tScubiteSpriteId;
        gSprites[spriteId].pos1.x = 100;
        gSprites[spriteId].pos1.y = 75;
        gSprites[spriteId].invisible = TRUE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tScubiteSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tScubiteSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateScubiteSprite(u8 a, u8 b)
{
    return CreatePicSprite2(SPECIES_MEADOSAUR, SHINY_ODDS, 0, 1, a, b, 14, -1, 0);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    scubiteSpriteId = NewGameBirchSpeech_CreateScubiteSprite(100, 0x4B);
    gSprites[scubiteSpriteId].callback = SpriteCB_Null;
    gSprites[scubiteSpriteId].oam.priority = 0;
    gSprites[scubiteSpriteId].invisible = TRUE;
    gTasks[taskId].tScubiteSpriteId = scubiteSpriteId;
...
static void NewGameBirchSpeech_ClearGenderWindow(u8 windowId, bool8 copyToVram)
{
    ...
    FillWindowPixelBuffer(windowId, PIXEL_FILL(MESSAGEBOX_BACKGROUND_COLOR));

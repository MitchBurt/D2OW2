...
void EvolutionScene(struct Pokemon* mon, u16 speciesToEvolve, bool8 canStopEvo, u8 partyID, u8 formIdToEvolve)
{
    ...
    DecompressPicFromTable(&gMonFrontPicTable[currentFormSpecies],
    ...
    DecompressPicFromTable(&gMonFrontPicTable[formSpeciesToEvolve],
...
static void CB2_EvolutionSceneLoadGraphics(void)
{
    ...
    DecompressPicFromTable(&gMonFrontPicTable[postEvoFormSpecies],
                             gMonSpritesGfxPtr->sprites[3],
                             postEvoFormSpecies);
...
static void CreateShedinja(u16 preEvoSpecies, struct Pokemon* mon)
{
    ...
    if (gEvolutionTable[preEvoSpecies][0].method == EVO_LEVEL_NINJASK && gPlayerPartyCount < 6)
    {
		...
		SetMonData(shedinja, MON_DATA_ABILITY_NUM, 0);
...
static void Task_EvolutionScene(u8 taskID)
{
    ...
    case 14: // congratulations string and rename prompt
        if (IsCryFinished())
        {
			...
			//ayylmao
			if(GetAbilityBySpecies(gTasks[taskID].tPostEvoSpecies, 
				GetMonData(mon, MON_DATA_ABILITY_NUM), 
				GetFormSpeciesId(gTasks[taskID].tPostEvoSpecies,gTasks[taskID].tPostEvoFormId)) == ABILITY_NONE)

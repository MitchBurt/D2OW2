...
static void CreatePartyMonIconSpriteParameterized(u16, u32, struct PartyMenuBox *, u8, u8);
...
static bool16 IsMonAllowedInParangeliaBerryPicking(struct Pokemon*);
...
//Egg Move Tutor --------------------------------------------------
static void Task_ChooseMonForEggTutor(u8);
static void CB2_ChooseMonForEggTutor(void);;
//Tm Move Tutor ---------------------------------------------------
static void Task_ChooseMonForTMTutor(u8);
static void CB2_ChooseMonForTMTutor(void);;
//Move Tutor ---------------------------------------------------
...
static void CursorCb_PkmFollow(u8); //tx_pokemon_follower
static void CursorCb_PkmUnfollow(u8); //tx_pokemon_follower
...
static bool8 ShowPartyMenu(void)
{
    ...
    case 21:
        ...
        break;
    case 22:
        BeginNormalPaletteFade(0xFFFFFFFF, 0, 16, 0, RGB_BLACK);//PALETTES_ALL
        gMain.state++;
        break;
    default:
...
#define SCROLLING_BACKGROUND_NUM 1
...
static void CreatePartyMonSprites(u8 slot)
{
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_MULTI_SHOWCASE && slot >= MULTI_PARTY_SIZE)
    {
        ...
        if (gMultiPartnerParty[actualSlot].species != SPECIES_NONE)
        {
            CreatePartyMonIconSpriteParameterized(gMultiPartnerParty[actualSlot].species, gMultiPartnerParty[actualSlot].personality, &sPartyMenuBoxes[slot], 0, 0); // handle forms
...
void Task_HandleChooseMonInput(u8 taskId)
{
    ...
    if (!gPaletteFade.active && MenuHelpers_CallLinkSomething() != TRUE)
    {
		...
		case 9: // Select button
            //gTasks[taskId].func = CursorCb_PkmAutomaticFollow;
            break;
        }
    }
...
void SetArceusForm(struct Pokemon *mon)
{
	...
	u16 formid = GetMonData(mon, MON_DATA_FORM_ID);
	u16 newformid;
    u16 forme;
     ...
     && ability == ABILITY_MULTITYPE)
    {
        ...
        SetMonData(mon, MON_DATA_FORM_ID, &newformid);
...
static void SetPartyMonsAllowedInMinigame(void)
{
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_MINIGAME)
    {
        ...
        {
            for (i = 0; i < gPlayerPartyCount; i++)
                *ptr += IsMonAllowedInParangeliaBerryPicking(&gPlayerParty[i]) << i;
        }
    }
}
...
static bool16 IsMonAllowedInParangeliaBerryPicking(struct Pokemon *mon)
{
    if (GetMonData(mon, MON_DATA_IS_EGG) != TRUE && GetMonData(mon, MON_DATA_SPECIES) == SPECIES_PARANGELIA)
        return TRUE;
    return FALSE;
}
...
static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname, struct PartyMenuBox *menuBox)
{
    ...
    if (species == SPECIES_NONE)
        return;
    if ((species == SPECIES_UNGA || species == SPECIES_NAUSEON) && StringCompare(nickname, gSpeciesNames[species]) == 0)
        return;
    switch (gender)
    {
    case MON_MALE:
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    u8 formId 			= GetMonData(&mons[slotId], MON_DATA_FORM_ID,  		 NULL);
	u16 formSpeciesId 	= GetFormSpeciesId(species, formId);
	bool8 canuseTeleport 	= FALSE;
	bool8 canuseSoftboiled 	= FALSE;
	bool8 canuseMilkDrink 	= FALSE;
...
//Nuzlocke
   ...
   if (GetMonData(&mons[slotId], MON_DATA_DEAD))
   {
    ...
    {
        switch(gLevelUpLearnsets[GetMonData(&mons[slotId], MON_DATA_SPECIES)][k].move){
			case MOVE_SOFT_BOILED:
				canuseSoftboiled = TRUE;
			break;
			case MOVE_TELEPORT:
				canuseTeleport = TRUE;
			break;
			case MOVE_MILK_DRINK:
				canuseMilkDrink = TRUE;
			break;
			case MOVE_SWEET_SCENT:
				canuseSweetScent = TRUE;
			break;
		}
    }
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, TRUE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, CB2_ReturnToUIMenu); //CB2_ReturnToFieldWithOpenMenu
}
...
static void CreatePartyMonIconSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox, u32 slot)
{
	...
	CreatePartyMonIconSpriteParameterized(species2, GetMonData(mon, MON_DATA_PERSONALITY), menuBox, 1, GetMonData(mon, MON_DATA_FORM_ID));
	UpdatePartyMonHPBar(menuBox->monSpriteId, mon);
}

static void CreatePartyMonIconSpriteParameterized(u16 species, u32 pid, struct PartyMenuBox *menuBox, u8 priority, u8 formId)
{
    if (species != SPECIES_NONE)
    {
        menuBox->monSpriteId = CreateMonIcon(species, SpriteCB_MonIcon, menuBox->spriteCoords[0], menuBox->spriteCoords[1], 4, pid, formId);
...
void Task_AbilityPatch(u8 taskId)
{
	...
	static const u8 askTextHidden[] = _("Would you like to change {STR_VAR_1}'s\nability for its Normal Ability?");
    static const u8 doneText[] = _("{STR_VAR_1}'s ability changed!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
    ...
    case 0:
		...
		if(tAbilityNum != 2)
			...
			StringExpandPlaceholders(gStringVar4, askTextHidden);
        PlaySE(SE_SELECT);
        DisplayPartyMenuMessage(gStringVar4, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
...
void Task_PowerCandy(u8 taskId)
{
    ...
    case 5:
		for(i = 0; i < 6; i++)
			SetMonData(mon, MON_DATA_HP_IV + i, &NewIv[i]);
...
{
    static const u8 askText[] = _("Do you want to slow down this Pokemon?");
    static const u8 doneText[] = _("Your Pokemon became slower!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
	struct Pokemon *mon = &gPlayerParty[tMonId];
	u8 NewSpd   =  0;
    ...
    case 4:
        ...
        break;
    case 5:
        SetMonData(mon, MON_DATA_SPEED_IV, &NewSpd);
		SetMonData(mon, MON_DATA_SPEED_EV, &NewSpd);
		UpdateMonDisplayInfoAfterRareCandy(tMonId, mon);
        RemoveBagItem(gSpecialVar_ItemId, 1);
        gTasks[taskId].func = Task_ClosePartyMenu;
        break;
...
#define tState        data[0]
#define tMonId        data[1]
#define tStattochange data[2]
#define tOldIVs       data[3]
#define tOldFunc    8

void Task_IVWings(u8 taskId)
{
    static const u8 askText[] = _("Do you want to power up this Pokemon?");
    static const u8 doneText[] = _("Your Pokemon became stronger!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
	struct Pokemon *mon = &gPlayerParty[tMonId];
	u8 newIvs   =  tOldIVs;
    ...
    case 4:
        ...
        break;
    case 5:
        SetMonData(mon, MON_DATA_HP_IV + tStattochange, &newIvs);
		UpdateMonDisplayInfoAfterRareCandy(tMonId, mon);
        RemoveBagItem(gSpecialVar_ItemId, 1);
        gTasks[taskId].func = Task_ClosePartyMenu;
        break;
...
void ItemUseCB_IVWings(u8 taskId, TaskFunc task)
{
    s16 *data = gTasks[taskId].data;

    tState = 0;
    tMonId = gPartyMenu.slotId;
	tStattochange = ItemId_GetSecondaryId(gSpecialVar_ItemId);
	tOldIVs = GetMonData(&gPlayerParty[tMonId], MON_DATA_HP_IV + tStattochange);
    SetWordTaskArg(taskId, tOldFunc, (uintptr_t)(gTasks[taskId].func));
    gTasks[taskId].func = Task_IVWings;
}
...
void Task_TypeGem(u8 taskId)
{
    static const u8 askText[] = _("Do you want to change your Pokemon\nHidden Type?");
    static const u8 doneText[] = _("Your Pokemon Hidden Power Type\nChanged!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
	struct Pokemon *mon = &gPlayerParty[tMonId];
	u8 extra = 1;
    ...
    case 0:
        // Can't use.
        if ((GetMonData(mon, MON_DATA_HP_IV)    < 30 ||
			...
			GetMonData(mon, MON_DATA_SPEED_IV) < 30) && !FlagGet(FLAG_PERFECT_IVS_MODE))
        {
            gPartyMenuUseExitCallback = FALSE;
            PlaySE(SE_SELECT);
            DisplayPartyMenuMessage(gText_WontHaveEffect, 1);
    ...
    case 4:
        ...
        break;
    case 5:
        SetMonData(mon, MON_DATA_HP_IV, 	&tnewHPIV);
		SetMonData(mon, MON_DATA_ATK_IV, 	&tnewAtkIV);
		SetMonData(mon, MON_DATA_DEF_IV, 	&tnewDefIV);
		SetMonData(mon, MON_DATA_SPATK_IV, 	&tnewSpAtkIV);
		SetMonData(mon, MON_DATA_SPDEF_IV, 	&tnewSpDefIV);
		SetMonData(mon, MON_DATA_SPEED_IV, 	&tnewSpdIV);
		UpdateMonDisplayInfoAfterRareCandy(tMonId, mon);
        RemoveBagItem(gSpecialVar_ItemId, 1);
        gTasks[taskId].func = Task_ClosePartyMenu;
        break;
...
void ItemUseCB_TypeGem(u8 taskId, TaskFunc task)
{
    s16 *data = gTasks[taskId].data;
    ...
    SetWordTaskArg(taskId, tOldFunc, (uintptr_t)(gTasks[taskId].func));
    gTasks[taskId].func = Task_TypeGem;
}
...
void Task_Exiolite(u8 taskId)
{
	...
	//Check if your Pokemon is able to Evolve
    ...
    case 4:
        ...
        break;
    case 5:
        SetMonData(mon, MON_DATA_EXIOLITE_ENABLED, &exioliteStatus);
...
void ItemUseCB_Exiolite(u8 taskId, TaskFunc task)
{
    s16 *data = gTasks[taskId].data;
    ...
    tMonId = gPartyMenu.slotId;
    SetWordTaskArg(taskId, tOldFunc, (uintptr_t)(gTasks[taskId].func));
    gTasks[taskId].func = Task_Exiolite;
}
...
#define tState          data[0]
#define tSpecies       data[1]
#define tCurrBall      data[2]
#define tMonId         data[3]
#define tOldFunc       4
#define tNewPokeball   data[6]


//static const u8 sText_AskSeal[] = _("Would you like to change {STR_VAR_1}'s\nPokeball to {STR_VAR_2}?");
//static const u8 sText_SealDone[] = _("{STR_VAR_1}'s Pokeball became\n{STR_VAR_2}!{PAUSE_UNTIL_PRESS}");
static const u8 sText_AskSeal[] = _("Would you like to change {STR_VAR_1}'s\nPokeball?");
static const u8 sText_SealDone[] = _("{STR_VAR_1}'s Pokeball\nChanged!{PAUSE_UNTIL_PRESS}");

static void Task_Seal(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
	u8 itemName[12];
    ...
    case 0:
        ...
        gPartyMenuUseExitCallback = TRUE;
        GetMonNickname(&gPlayerParty[tMonId], gStringVar1);
		//GetMonNickname(&gPlayerParty[tMonId], gStringVar2);
        //StringCopy(gStringVar2, gNatureNamePointers[tNewNature]);
        StringExpandPlaceholders(gStringVar4, sText_AskSeal);
        PlaySE(SE_SELECT);
        DisplayPartyMenuMessage(gStringVar4, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
    ...
    case 3:
        PlaySE(SE_USE_ITEM);
        StringExpandPlaceholders(gStringVar4, sText_SealDone);
        DisplayPartyMenuMessage(gStringVar4, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
        break;
    case 4:
        ...
        break;
    case 5:
        SetMonData(&gPlayerParty[tMonId], MON_DATA_POKEBALL, &tNewPokeball);
        CalculateMonStats(&gPlayerParty[tMonId]);
...
void ItemUseCB_Seal(u8 taskId, TaskFunc task)
{
    ...
    tMonId = gPartyMenu.slotId;
    tSpecies = GetMonData(&gPlayerParty[tMonId], MON_DATA_SPECIES, NULL);
	tCurrBall = GetMonData(&gPlayerParty[tMonId], MON_DATA_POKEBALL, NULL);
    tNewPokeball = ItemId_GetSecondaryId(gSpecialVar_ItemId);
    SetWordTaskArg(taskId, tOldFunc, (uintptr_t)(gTasks[taskId].func));
    gTasks[taskId].func = Task_Seal;
}
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
	...
	u8 usedCandies = 0;
	usedCandies = VarGet(VAR_USED_RARE_CANDIES);
    ...
    if (GetMonData(mon, MON_DATA_LEVEL) != MAX_LEVEL)
    {
		...
		VarSet(VAR_USED_RARE_CANDIES, usedCandies+1);
    }
    else
    {
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    if (targetSpecies != SPECIES_NONE)
    {
        ...
        BeginEvolutionScene(mon, targetSpecies, 1, gPartyMenu.slotId, targetFormId);
        DestroyTask(taskId);
    }
    else
    {
...
static void TryChangeShayminForm(u8 taskId, TaskFunc task)
{
        ...
        && (gLocalTime.hours >= 12 && gLocalTime.hours < 24))
    {
		...
		formId = GetFormIdFromFormSpeciesId(targetSpecies);
        ...
        CreatePartyMonIconSpriteParameterized(targetSpecies, GetMonData(mon, MON_DATA_PERSONALITY), &sPartyMenuBoxes[gPartyMenu.slotId], 0, formId);
        CalculateMonStats(mon);
        GetMonNickname(mon, gStringVar1);
...
{
	if(!FlagGet(FLAG_DOUBLE_BATTLE_MODE))
		InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, CB2_ReturnToUIMenu); //CB2_ReturnToFieldWithOpenMenu
	else
		InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_DOUBLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, CB2_ReturnToUIMenu);
}
...
static u8 CheckBattleEntriesAndGetMessage(void)
{
    ...
    for (i = 0; i < maxBattlers - 1; i++)
    {
        u16 species = GetBaseFormSpeciesId(GetMonData(&party[order[i] - 1], MON_DATA_SPECIES));
        u16 item = GetMonData(&party[order[i] - 1], MON_DATA_HELD_ITEM);
        for (j = i + 1; j < maxBattlers; j++)
        {
            if (species == GetBaseFormSpeciesId(GetMonData(&party[order[j] - 1], MON_DATA_SPECIES)))
...
//Tm Move Tutor ---------------------------------------------------
void ChooseMonForTMTutor(void)
{
    ScriptContext2_Enable();
    FadeScreen(FADE_TO_BLACK, 0);
    CreateTask(Task_ChooseMonForTMTutor, 10);
}
...
static void Task_ChooseMonForTMTutor(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        CleanupOverworldWindowsAndTilemaps();
        InitPartyMenu(PARTY_MENU_TYPE_MOVE_RELEARNER, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_AND_CLOSE, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, CB2_ChooseMonForTMTutor);
        DestroyTask(taskId);
    }
}
//Move Tutor ---------------------------------------------------
...
static void CursorCb_PkmFollow(u8 taskId)
{
	...
	if(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_ON_FOOT){
		...
		POF_DestroyFollower();
		gSaveBlock2Ptr->pokemonFollower.partySlotId = (gPartyMenu.slotId + 1);
...
void CursorCb_PkmAutomaticFollow(void)
{

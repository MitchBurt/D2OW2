...
void InitAndLaunchChosenStatusAnimation(bool8 isStatus2, u32 status)
{
    ...
    if (!isStatus2)
    {
        ...
        else if (status == STATUS1_POISON || status & STATUS1_TOXIC_POISON || status & STATUS1_INTOXICATE)
...
static void BattleLoadMonSpriteGfx(struct Pokemon *mon, u32 battlerId, bool32 opponent)
{
    ...
    if (species == SPECIES_CASTFORM || species == SPECIES_NOTYUHDAD)
    {
        paletteOffset = 0x100 + battlerId * 16;
        LZDecompressWram(lzPaletteData, gBattleStruct->castformPalette[0]);
        LoadPalette(gBattleStruct->castformPalette[gBattleMonForms[battlerId]], paletteOffset, 0x20);
    }
	/*/else if(species == SPECIES_KECLEON)
    {
		intesity = 1+(GetMonData(mon, MON_DATA_LEVEL)/20);
		
		switch(gBattleMons[battlerId].type1){
			case TYPE_ROCK:
			case TYPE_GROUND:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB(15, 8, 0));//Brown
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
			case TYPE_WATER:
			case TYPE_ICE:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB_BLUE);
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
			case TYPE_GRASS:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB_GREEN);
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
			case TYPE_FIRE:
			case TYPE_FIGHTING:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB_RED);
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
			case TYPE_GHOST:
			case TYPE_POISON:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB_PURPLE);
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
			case TYPE_FAIRY:
			case TYPE_PSYCHIC:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB(31, 21, 24));//Pink
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
			case TYPE_DARK:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB_BLACK);
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
			case TYPE_ELECTRIC:
				BlendPokemonPalette(paletteOffset, 16, intesity, RGB_YELLOW);
				CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
			break;
		}
    }/*/
    ...
    if (gBattleSpritesDataPtr->battlerData[battlerId].transformSpecies != SPECIES_NONE && species != SPECIES_KECLEON)
    {
        ...
        if(GET_BATTLER_SIDE(battlerId) == B_SIDE_PLAYER){
            SetCustomMonPalette(&gPlttBufferUnfaded[paletteOffset], mon);
            SetCustomMonPalette(&gPlttBufferFaded[paletteOffset], mon);
        }
        else{
            SetEnemyCustomMonPalette(&gPlttBufferUnfaded[paletteOffset], mon);
            SetEnemyCustomMonPalette(&gPlttBufferFaded[paletteOffset], mon);
        }
        CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);	
	}
}
...
{
    u8 position = GetBattlerPosition(battlerId);//asdf
    DecompressPicFromTable(&gTrainerFrontPicTable[frontPicId],
                           gMonSpritesGfxPtr->sprites[position],
                           SPECIES_NONE);
    LoadTrainerSpritePalette(&gTrainerFrontPicPaletteTable[frontPicId]);
}

void DecompressPlayerTrainerFrontPic(u16 frontPicId, u8 battlerId)
{
    u8 position = GetBattlerPosition(battlerId);
    DecompressPicFromTable(&gTrainerFrontPicTable[frontPicId],
                           gMonSpritesGfxPtr->sprites[position],
                           SPECIES_NONE);
    LoadPlayerSpritePaletteDayNight(&gTrainerFrontPicPaletteTable[frontPicId]);
}
...
void HandleSpeciesGfxDataChange(u8 battlerAtk, u8 battlerDef, bool8 notTransform, bool32 megaEvo)
{
    ...
    else
    {
        ...
        if (targetSpecies == SPECIES_CASTFORM || targetSpecies == SPECIES_NOTYUHDAD)
        {
